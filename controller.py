...
class Controller:
    ...
    def check_user_answer(self, answer):
        if answer == "1":
            ...
            try:
                article = self.article_model.get_single_article(article_title)  # достаем из модели
            except ValueError:
                self.user_interface.show_incorrect_title_error(article_title)  # обработка ошибки
            else:
                self.user_interface.show_single_article(article)  # покажем, то что достали
        elif answer == "4":
            article_title = self.user_interface.get_user_article()  # пользователь говорит, что хочет удалить
            try:
                title = self.article_model.remove_article(article_title)  # бд находит статью и удаляет
            except ValueError:
                self.user_interface.show_incorrect_title_error(article_title)  # обработка ошибки
            else:
